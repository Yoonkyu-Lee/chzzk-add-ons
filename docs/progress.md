...
T12 — 문서 완료. 단위 95/95 + 하니스 10/10 @ ccd04ae. main 머지 대기.
...
| T12 | README·수동 시나리오·머지 | `[~]` | | 문서 완료, 머지 대기 |
...
| 확인 시각 | SHA | 결과 |
|---|---|---|
| 2026-09-15 03:40 | `ccd04ae` | 95/95 통과 |
...
| # | 시나리오 | 상태 | 확인 시각 | SHA |
|---|----------|------|-----------|-----|
| 1 | 목록 페이지에 툴바·카드 버튼 주입 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 2 | 전체 담기 → 큐가 오래된 순으로 채워짐 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 3 | 패널 열림 + 순서 변경 저장 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 4 | 새로고침 후 큐·패널 상태 복원 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 5 | SPA 내부 이동 후 패널 생존 (D1 핵심 가정) | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 6 | `currentTime` 쓰기로 seek 동작 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 7 | 종료 10초 전 다음 항목 카드 + 전환 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 8 | 이어보기: seek → 재로드 → 위치 복원 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 9 | 큐에 없는 영상 페이지에서 무간섭 | 통과 | 2026-09-15 03:40 | `ccd04ae` |
| 10 | 큐 버전 불일치 → 패널이 알린다 | 통과 | 2026-09-15 03:40 | `ccd04ae` |

### 완료 기준 (스펙 10장)

| # | 조건 | 상태 | 근거 |
|---|------|------|------|
| 1 | `node --test` 전부 통과 | 충족 | 95/95 @ `ccd04ae` |
| 2 | 하니스 시나리오 통과 + 증빙 | 충족 | 10/10 @ `ccd04ae`, `tools/out/result.json` |
...
| 8 | 에러 경로 중 재현 가능한 것 실제 확인 | **부분 충족** | 아래 참고 |
...
- 실제 API 장애 시 문구 — 네트워크를 끊는 방식으로는 재현했으나 하니스에
  넣지 않았다
